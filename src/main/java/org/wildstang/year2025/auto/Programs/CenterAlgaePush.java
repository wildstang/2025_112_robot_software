...
/**
 */
public class CenterAlgaePush extends AutoProgram {

    @Override
    protected void defineSteps() {
        SwerveDrive swerve = (SwerveDrive) Core.getSubsystemManager().getSubsystem(WsSubsystems.SWERVE_DRIVE);

        addStep(new SwervePathFollowerStep("TushPush", swerve, true));

        AutoParallelStepGroup group1 = new AutoParallelStepGroup();
        group1.addStep(new SetArmLiftStateStep(GameStates.L2_ALGAE_REEF, true));
        group1.addStep(new SwervePathFollowerStep("CenterStartToGH", swerve));
